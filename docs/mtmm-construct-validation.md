...
**See also:** [Relibility vs. Validity]
...
## Why These Patterns Arise (Sketch)
Let observed score \(X_{tm} = T_t + M_m + E_{tm}\). Under simplifying assumptions (trait, method, error uncorrelated):
- \(\operatorname{Var}(X_{tm}) = \sigma^2_{T_t} + \sigma^2_{M_m} + \sigma^2_{E_{tm}}\)
- **Reliability** relates to the share of trait variance in \(X_{tm}\).
